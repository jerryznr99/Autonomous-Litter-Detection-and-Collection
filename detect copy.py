...
vl53l0x = serial.Serial(port = "/dev/tty.usbmodem1201", baudrate = 9600, timeout = .1)

ssc32 = serial.Serial('/dev/tty.usbserial-AB0NBQKD', 9600)

from roboclaw_3 import Roboclaw
roboclaw = Roboclaw("/dev/tty.usbmodem1301", 9600)
roboclaw.Open()
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if (save_img or save_crop or view_img) and conf > 0.5:  # Add bbox to image
                    ...
                    if count > 0:
                        if position == "centered":
                            if distance <= 325 and distance > 0:
...
def direction(xyxy):
    ...
    if box_center_x > frame_center_x + 70:
        result = "right"
    elif box_center_x < frame_center_x - 70:
        result = "left"
    else:
        result = "centered"
...
def Distance_finder():
    ...
    while vl53l0x.in_waiting > 0:
            
        serialString = vl53l0x.readline()

        serialString = serialString.decode()

        reading = int(serialString.strip())

    return reading

    # real_width = 63.5 #66.2
    ...
    # face_width_in_frame = xyxy[2] - xyxy[0]
...
#top left (0,0) + bottom right (1920, 1080) = middle (960, 540)
#xyxy is a list, each element is torch.Tensor (its different way of storing a number)
